...
export const DatabaseNode = memo(({ data, selected, id }: NodeProps) => {
  ...
  const name = nameProperty?.value || data.label || 'Database';
  const type = typeProperty?.value || 'PostgreSQL';
  ...
  const getStatusBorder = () => {
    switch (status.status) {
      ...
      default:
        return 'border-border hover:border-node-database/50';
    }
  };
  ...
  return (
    ...
    `}>
      ...
      <Handle
        ...
        className="w-3 h-3 !bg-node-database border-2 border-card"
      />
      
      <div className="p-3">
        <div className="flex items-center gap-2 mb-2">
          <div className="p-1.5 rounded-md bg-node-database/20">
            <Database className="h-4 w-4 text-node-database" />
          </div>
        ...
        {status.metrics && (
          <div className="mt-2 pt-2 border-t border-border space-y-1">
            ...
            <div className="flex justify-between text-xs">
              ...
              <span>{Math.floor(status.metrics.requests)}</span>
            </div>
          </div>
        )}
      </div>
